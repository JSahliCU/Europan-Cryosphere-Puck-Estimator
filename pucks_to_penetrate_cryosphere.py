...
def evaluate_number_of_pucks_on_arbitrary_europa(
    ...
    if landing_site == "sub jovian point":
        ...
        raise (
            ValueError(
                "landing_site must be ['sub jovian point', 'anti jovian point', 'anti orbital point']"
            )
        )
    ...
    n_i = np.sqrt(
        eim.cryosphere_model_df.iloc[-1]["epsilon_s_prime"] * epsilon_0 * mu_0
    )
    ...
    (
        ...
        eim,
        ag,
        T_Bh_ocean_high_band,
        T_Bv_ocean_high_band,
        uhf_ant.directivity_pattern_RHCP,
    )
...
def calc_path_loss(omega, eta_vac, epsilon_s_prime, epsilon_s_primeprime, sigma_s):
    ...
    epsilon_s = epsilon_0 * (
        epsilon_s_prime - 1j * (epsilon_s_primeprime + sigma_s / (epsilon_0 * omega))
    )
    ...
    return (
        (omega / np.sqrt(2))
        * np.sqrt(epsilon_0 * mu_0)
        * np.sqrt(
            np.sqrt(epsilon_s_prime**2 + (epsilon_s_primeprime) ** 2) - epsilon_s_prime
        )
    )


def calc_gamma_ab(
    ag, f, epsilon_i_prime, epsilon_t_prime, sigma, H, bistatic_polarization, torr
):
...
def integrate_gamma_ab_and_T_Ba_i(
    ag, T_Bv_i, T_Bh_i, f, epsilon_i_prime, epsilon_t_prime, sigma, H, torr
):
    d_solid_angle = ag.d_solid_angle[None, :, :]

    gamma_ab_helper = lambda x: calc_gamma_ab(
        ag, f, epsilon_i_prime, epsilon_t_prime, sigma, H, x, torr
    )
...
def calc_welling_noise_stream(direction, f, eim, ag, T_Bh, T_Bv, directivity_RHCP):
    if direction == "down":
    ...
    for d in depth_list:
        ...
        alpha = calc_path_loss(
            2 * np.pi * f, eta_vac, epsilon_s_prime, epsilon_s_primeprime, sigma_s
        )
        ...
        temp = (
            (T_Bh_d + T_Bv_d).reshape(ag.theta_grid.shape)
            * directivity_RHCP
            * np.sin(np.deg2rad(ag.theta))[:, None]
        )
        temp = scipy.integrate.simpson(temp, x=np.deg2rad(ag.phi_grid))
        T_A[d] = (1 / (4 * np.pi)) * scipy.integrate.simpson(
            temp, x=np.deg2rad(ag.theta)
        )
...
class europa_ice_model:
    def __init__(
        ...
        self.cryosphere_model_df = pd.DataFrame(
            {"Depth (m)": np.arange(0, self.D_total + delta_d, delta_d)}
        )
        self.cryosphere_model_df["Temperature (K)"] = self.cryosphere_model_df[
            "Depth (m)"
        ].map(self.temperature_at_depth)
        self.cryosphere_model_df["Porosity (m^3/m^3)"] = self.cryosphere_model_df[
            "Depth (m)"
        ].map(self.porosity_at_depth)
        self.cryosphere_model_df["Salt fraction (kg/kg)"] = self.cryosphere_model_df[
            "Depth (m)"
        ].map(self.salt_fraction_at_depth)
        ...
        self.cryosphere_model_df["epsilon_s_primeprime"] = 10 ** (
            -3.0129 + 0.0123 * (T - 273.13)
        )

        # Estimate the conductivity of the ice
        molar_mass_salt = (35.453 + 22.990) / 1000  # kg/mol
        molarity_salt = (
            self.cryosphere_model_df["Salt fraction (kg/kg)"] / molar_mass_salt
        )
    ...
    def temperature_at_depth(self, d):
        if d > self.D_cond + self.D_conv:
            ...
            m, b = math_funcs.linear_fit(0, self.T_u, self.D_cond, self.T_l)
            return m * d + b
...
def estimate_puck_placement(
    ...
    for d in np.arange(len(eim.cryosphere_model_df)):
        ...
        ant_backside_exists = antenna_pattern.directivity(180, 0, temperature_ice) > 0

        alpha = calc_path_loss(
            2 * np.pi * communication_frequency,
            eta_vac,
            epsilon_s_prime,
            epsilon_s_primeprime,
            sigma_s,
        )
        ...
        def calc_prob_error(
            ...
            received_power = (
                ...
                * attenuation
                * (lambda_s_transmitter / lambda_s_receiver)
            )

            ant_temp = (
                rad_eff * match_eff * T_A_upwelling
            ...
            if use_shannon_channel_limit:
                if (
                    2 ** (data_rate / communication_bandwidth) - 1
                ...
                CNR_per_bit = (
                    data_rate / communication_bandwidth
                ) * signal_to_noise_ratio
                probability_of_error = probability_of_error_for_MFSK(
                    CNR_per_bit, symbols
                )
        ...
        if d == 0 or d == len(eim.cryosphere_model_df) - 1:
            ...
            if (
                lower_prob_error > limit_probability_of_error
    ...
    if record_puck_placement_data:
        recorded_data = pd.DataFrame(
            {"Placement Depth (m)": placement_depths, "Attenuation": attenuations}
        )
