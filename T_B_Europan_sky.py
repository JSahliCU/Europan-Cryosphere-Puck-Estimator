...
class angle_grid:
    def __init__(self, dtheta, dphi):
        ...
        self.phi_grid, self.theta_grid = np.meshgrid(self.phi, self.theta)

    def get_angle_grid(self):
        return self.theta, self.phi, self.phi_grid, self.theta_grid


def deconvolve_Cecconi_T_A_to_T_B(ag, W_h, W_v, T_A):
    theta, phi, phi_grid, theta_grid = ag.get_angle_grid()
...
def resample_course(T_B, return_grids=False):
    ...
    theta_resample, phi_resample, phi_grid_resample, theta_grid_resample = angle_grid(
        10, 10
    ).get_angle_grid()
...
if __name__ == "__main__":
    ...
    print(
        f"Percent error in deconvolve, convolve method: {(T_A - T_A_gal_low_band) * 100}%"
    )
    ...
    for func, title in [
        ...
        T_Bv_low_band, T_Bh_low_band, T_Bv_high_band, T_Bh_high_band = func()

        # The course grid
        theta, phi, phi_grid, theta_grid = angle_grid(10, 10).get_angle_grid()
        ...
        plt.scatter(
            U,
            V,
            c=10 * np.log10(T_Bv_low_band.reshape(theta_grid_rad.shape)[0:9].flatten()),
            cmap="viridis",
            vmin=0,
            vmax=200,
        )
        ...
        plt.savefig(
            title
            + " Northern UV Projection of Vertical Brightness Temperature Low Band.pdf"
        )
        ...
        plt.scatter(
            ...
            V,
            c=10
            * np.log10(T_Bv_high_band.reshape(theta_grid_rad.shape)[0:9].flatten()),
            cmap="viridis",
            vmin=0,
            vmax=200,
        )
        ...
        plt.savefig(
            title
            + " Northern UV Projection of Vertical Brightness Temperature High Band.pdf"
        )
